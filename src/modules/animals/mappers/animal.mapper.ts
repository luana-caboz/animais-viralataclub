import { AnimalDB } from "@/modules/animals/types/animal-db";
import { InternalAnimal } from "@/modules/animals/types/animal";
...
export function mapAnimal(
  ...
  return {
    ...
    fotos: (animal.animal_images ?? []).map((image) => ({
      id: image.id,
      animalId: image.animal_id,
      url: image.url,
      cloudinaryPublicId: image.cloudinary_public_id ?? undefined,
      driveFileId: image.drive_file_id ?? undefined,
      driveMd5: image.drive_md5 ?? undefined,
      driveModifiedTime: image.drive_modified_time ?? undefined,
      legenda: image.legenda ?? undefined,
      ordem: image.ordem,
